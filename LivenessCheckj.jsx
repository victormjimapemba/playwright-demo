...
function LivenessCheck() {
  ...
  const fetchChallenges = async () => {
    const res = await fetch(`${API_BASE}/api/liveness/new-session`, { method: "POST" });
    if (!res.ok) throw new Error(`new-session failed: ${res.status}`);
    return res.json();   // { session_id, challenges }
  };
  ...
  const handleStart = async () => {
    ...
    let session_id, ch;
    try {
      const session = await fetchChallenges();
      session_id = session.session_id;
      ch         = session.challenges;
    } catch (e) {
      setCamError("Could not reach server. Is the backend running?");
      return;
    }
  ...
  const handleRecordingDone = async (sid, ch) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/liveness/verify`, {
        method: "POST",
        body: form,
      });
      ...
      // data = { live, challenges, spoof_check, error }

      if (data.error) {
        setCamError(data.error);
        setStage("failure");
        setResult({ live: false, challenges: data.challenges ?? ch.map(name => ({ name, passed: false, detail: data.error })) });
      } else {
        setResult({ live: data.live, challenges: data.challenges });
        setStage(data.live ? "success" : "failure");
      }
    } catch (err) {
      setCamError(`Verification failed: ${err.message}`);
      setResult({ live: false, challenges: ch.map(name => ({ name, passed: false, detail: "Request error" })) });
      setStage("failure");
    }
